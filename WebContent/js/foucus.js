$(function() {
    ...
    $("#slider .btn span").css("opacity",0.4).mouseover(function() {
        index = $("#slider .btn span").index(this);
        showPics(index);
    }).eq(0).trigger("mouseover");

    //上一页、下一页按钮透明度处理
    $("#slider .preNext").css("opacity",0.2).hover(function() {
    ...
    function showPics(index) { //普通切换
        ...
        $("#slider .btn span").stop(true,false).animate({"opacity":"0.4"},300).eq(index).stop(true,false).animate({"opacity":"1"},300); //为当前的按钮切换到选中的效果
    }
});/**
