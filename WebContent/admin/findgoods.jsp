...
<body>
	...
	一级分类：<select class="span2" style="width:160px" name="suptypeid" id="goods" onchange="changeGoods()" required>
							<option value="${suptypeid}">当前：<script>selectgoods_supertypes('${suptypeid}')</script></option>
							...
							二级分类：<select class="span2" style="width:150px" name="subtypeid" id="subgoods" required>
								<option value="${subtypeid}">当前：<script>selectgoods_subtypes('${subtypeid}')</script></option>
								
							</select>
	上下架状态：<select name="shelves">
		<option value="${shelves}">当前选择的是：<script>selectshelves('${shelves}')</script></script></option>
	...
	<input	type="submit" class="search_btn" value="查找" />
		...
		<div id="all">
			...
			<tr>	
				<td><a href="admin/adminshowGoods!findgoodsspecial1?goodspage.currentPage=1&&goodsname=${goodsname}&&suptypeid=${suptypeid}&&subtypeid=${subtypeid}&&shelves=${shelves}">首页</a></td>
				<s:iterator value="goodspage.pageList" id="e">
					<td><a href="admin/adminshowGoods!findgoodsspecial1?goodspage.currentPage=${e}&&goodsname=${goodsname}&&suptypeid=${suptypeid}&&subtypeid=${subtypeid}&&shelves=${shelves}">${e}</a></td>
				</s:iterator>
				<td><a href="admin/adminshowGoods!findgoodsspecial1?goodspage.currentPage=${goodspage.totalPage}&&goodsname=${goodsname}&&suptypeid=${suptypeid}&&subtypeid=${subtypeid}&&shelves=${shelves}">末页</a></td>	
