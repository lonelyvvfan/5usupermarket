...
<div id="win">
		...
		<form action="admin/adminshowOrder!findorderspecial" method="post">
			用户名：<input type="text" class="search_box" name="username" placeholder="请输入用户名" /> 
			订单号：<input type="text" class="search_box" name="orderNo" placeholder="请输入订单号" />&nbsp;&nbsp;
					...
					<tbody>
						<s:iterator value="orderpage.list">
							...
							<td><input type="checkbox" name="checkbox" value="${id}"></td>
								...
								<td>${other_address}<script>selectschool('${school}')</script><br><script>selectliving('${living}')</script>&nbsp;${doorplate}楼&nbsp;${room}室</td>						
								<td>${orderday}</td>
							...
							<tr>	
								<td>
									<a href="admin/adminshowOrder?orderpage.currentPage=1">首页</a>
								</td>
								<s:iterator value="orderpage.pageList" id="e">
									<td>
										<a href="admin/adminshowOrder?orderpage.currentPage=${e}">${e}</a>
									</td>
								</s:iterator>
							<td>
								<a href="admin/adminshowOrder?orderpage.currentPage=${orderpage.totalPage}">末页</a>
							</td>	
