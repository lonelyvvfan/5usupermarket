...
<div id="win">
	...
	<input type="text" class="s_search_box" name="goodsname" />
	...
	<input	type="submit" class="search_btn" value="查找" />
		...
		<div id="s_all">
				<table  class="m_table"  border="0" cellspacing="0" cellpadding="0"
					...
					<thead>
					<input class="btn btn-danger all_option" type="submit" value="删除" onclick=seller_secGoodDeleteAll_comfirm()   />
					<!-- <input class="btn s_btn-update all_option" type="submit" value="修改" />
					 -->
					<input class="btn btn-info all_option" type="submit" value="上架"   />
					<input class="btn btn-warning all_option" type="submit" value="下架" />
			...
			<tr>	
				<td><a href="seller/sellershowSeller!findtakeoutspecial?takeoutpage.currentPage=1&&sellerid=${sellerid}&&goodsname=${goodsname}&&suptypeid=${suptypeid}&&subtypeid=${subtypeid}">首页</a></td>
				<s:iterator value="takeoutpage.pageList" id="e">
					<td><a href="seller/sellershowSeller!findtakeoutspecial?takeoutpage.currentPage=${e}&&sellerid=${sellerid}&&goodsname=${goodsname}&&suptypeid=${suptypeid}&&subtypeid=${subtypeid}">${e}</a></td>
				</s:iterator>
				<td><a href="seller/sellershowSeller!findtakeoutspecial?takeoutpage.currentPage=${goodspage.totalPage}&&sellerid=${sellerid}&&goodsname=${goodsname}&&suptypeid=${suptypeid}&&subtypeid=${subtypeid}">末页</a></td>	
				<td>共有${takeoutpage.totalPage}页，${takeoutpage.allRow}条记录</td>
				</tr>

		<a href="seller/sellershowSeller!takeoutshow?sellerid=${sellerid}">返回</a>
	</div>
	</div>
</div>
</div>
